...
#define MPU6050WRITE_REG(reg, data)                 MYI2C_Write_Reg(&mpu6050_i2cbus, reg, data)                // 可以替换为你的写寄存器函数
#define MPU6050READ_REG(reg)                        MYI2C_Read_Reg(&mpu6050_i2cbus, reg)                       // 可以替换为你的读寄存器函数
#define MPU6050_READ_REG_CONTINUE(reg, len, P_data) MYI2C_Read_Reg_Continue(&mpu6050_i2cbus, reg, len, P_data) // 可以替换为你的连续读写寄存器函数
//////////////////////////////////////以上为方便移植的接口///////////////////////////////////////////////////////
...
void MPU6050_init(GPIO_TypeDef *GPIOx, uint16_t SCl, uint16_t SDA)
{
    ...
    MPU6050_init_Struct.Filter      = Band_96Hz;     // 滤波器带宽(原21Hz太慢,陀螺仪延迟8.5ms→96Hz仅2.9ms)
    ...
    PT1Filter_InitWithFreq(&pt1_filter_gx, 120, MPU6050_init_Struct.SMPLRT_Rate);
    PT1Filter_InitWithFreq(&pt1_filter_gy, 120, MPU6050_init_Struct.SMPLRT_Rate);
    PT1Filter_InitWithFreq(&pt1_filter_gz, 120, MPU6050_init_Struct.SMPLRT_Rate);
    // //以下为卡尔曼
