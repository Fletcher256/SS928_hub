...
HeadingPID_t headingPID =
{
	...
	.MaxI         = 5.0f,
	.MaxOut       = 8.0f,
	.Deadband     = 3.0f,    // 放宽死区, 滤除MPU6050噪声
	.D_Alpha      = 0.88f,   // 加强D项低通, 抑制高频抖动
	.SmoothAlpha  = 0.2f,      // 输出平滑: 20%新值+80%旧值, 强抗抖
	.CrossTrackKp     = 2.0f,
...
void Odometry_Reset(void)
{
	odom.x = 0.0f;
	odom.y = 0.0f;
	odom.theta = 1.5707963f;   // π/2, 车头指向Y轴正方向(前方), 直行时y增长x不变
	odom.distance = 0.0f;
}
...
void HeadingPID_Init(HeadingPID_t *p)
{
	...
	p->MaxOut       = 8.0f;
	p->Deadband     = 0.9f;
	p->D_Alpha      = 0.88f;   // 加强D项低通, 抑制高频抖动
	p->SmoothAlpha  = 0.2f;    // 强抗抖
	p->CrossTrackKp     = 2.0f;
